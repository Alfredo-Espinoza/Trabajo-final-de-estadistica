...
## FACTORIAL EXPLORATIVA 

Calculando la matriz de correlación 
```{r}
theData=deturb[-c(1,17)] 
```
...
```{r}
library(GPArotation)
resfa <- fa(theData,nfactors = 1,cor = 'mixed',rotate = "varimax",fm="minres") #SOLO SE CAMBIA EL NFACTORS
...
```{r}
resfa$TLI
#si el índice de TUCKER LEWIS  #ES MALO es menor a 0.9 
```
QUÉ VARIABLES APORTARON MÁS A LOS FACTORES? 
```{r}
sort(resfa$communality) #CUALES SEAN UTILIZADO BIEN, ESTO SE PODÍA VER EN EL KMO #COBERTURA DE SALUD 
```
...
#K=6 cantidad de clusters para cada tipo
...
```{r}
aggregate(Roadquality~clusterPT,data=teste,FUN = mean)
```
