...
El recomendado es diana, por tener mayor average silhoutte width
...
El cluster 3 es el mejor. El orden de mejor a peor es de 3, 1, 4, 2
...
```{r}
teste=merge(mapaMundo,deturb,by.x='COUNTRYAFF',by.y='CountryName', all.x=T,all.y=T)
```
...
Recuperando nombres para el mapa 

```{r}
deturb[deturb$CountryName=="Congo, Dem. Rep.","CountryName"]="Democratic Republic of the Congo"
deturb[deturb$CountryName=="Cabo Verde","CountryName"]="Cape Verde"
deturb[deturb$CountryName=="Cote d'Ivoire","CountryName"]="Côte d'Ivoire"
deturb[deturb$CountryName=="Gambia, The","CountryName"]="Gambia"
deturb[deturb$CountryName=="Iran, Islamic Rep.","CountryName"]="Iran"
deturb[deturb$CountryName=="Kyrgyz Republic","CountryName"]="Kyrgyzstan"
deturb[deturb$CountryName=="Lao PDR","CountryName"]="Laos"
deturb[deturb$CountryName=="Slovak Republic","CountryName"]="Slovakia"
```

```{r}
CountryFull=merge(mapaMundo,deturb,by.x='COUNTRYAFF',by.y='CountryName')
```
...
##Se procederá a graficar para ver las posiciones de los países:
```{r}
aggregate(desarrolloecon~clustDIV,data=CountryFull,FUN = mean)
```

```{r,fig.width=10, fig.height=5} 
# colores
myColors=c('red','skyblue',"darkgreen","purple") 
...
Se hace esta regresión porque la dependiente es binaria.
...
Debemos averigar si predice razonablemene bien
...
No parece tener mucha capacidad predictiva
